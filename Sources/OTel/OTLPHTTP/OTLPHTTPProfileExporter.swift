...
final class OTLPHTTPProfileExporter: OTelProfileExporter {
    typealias Request = Opentelemetry_Proto_Collector_Profiles_V1development_ExportProfilesServiceRequest
    typealias Response = Opentelemetry_Proto_Collector_Profiles_V1development_ExportProfilesServiceResponse
    ...
    init(configuration: OTel.Configuration.OTLPExporterConfiguration, logger: Logger) throws {
        self.logger = logger.withMetadata(component: "OTLPHTTPProfileExporter")
        var configuration = configuration
        configuration.endpoint = configuration.profilesHTTPEndpoint
    ...
    func export(_ batch: some Collection<Opentelemetry_Proto_Profiles_V1development_ResourceProfiles> & Sendable) async throws {
        ...
        if response.hasPartialSuccess {
            ...
            if response.partialSuccess.rejectedProfiles == 0, response.partialSuccess.errorMessage.isEmpty {
                logLevel = .debug
            } else {
                logLevel = .warning
            }
            logger.log(level: logLevel, "Partial success", metadata: [
                ...
                "rejected_profiles": "\(response.partialSuccess.rejectedProfiles)",
            ])
        }
    }
