...
final class OTelMetricExporterTests: XCTestCase {
    func test_MultiplexExporter_forwadsCallsToAllExporters() async throws {
        ...
        for exporter in recordingExporters {
        ...
        for exporter in recordingExporters {
            exporter.assert(exportCallCount: 2, forceFlushCallCount: 1, shutdownCallCount: 0)
        }

        await multiplexExporter.shutdown()
        for exporter in recordingExporters {
            exporter.assert(exportCallCount: 2, forceFlushCallCount: 1, shutdownCallCount: 1)
        }
    }
}
