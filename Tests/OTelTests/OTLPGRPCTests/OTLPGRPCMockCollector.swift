...
final class OTLPGRPCMockCollector: Sendable {
    ...
    @discardableResult
    static func withInsecureServer<T>(
        operation: (_ collector: OTLPGRPCMockCollector, _ endpoint: String) async throws -> T
    ) async throws -> T {
        let collector = self.init()
        let server = GRPCServer(
            transport: .http2NIOPosix(address: .ipv4(host: "127.0.0.1", port: 0), transportSecurity: .plaintext),
            services: [
                collector.recordingLogsService, collector.recordingMetricsService, collector.recordingTraceService,
            ]
        )
